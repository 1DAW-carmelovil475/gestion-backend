...
app.get('/api/auth/me', authGuard, (req, res) => {
    res.json({ id: req.user.id, email: req.user.email, nombre: req.user.nombre, rol: req.user.rol });
});
...
app.post('/api/usuarios', authGuard, adminGuard, async (req, res) => {
    ...
    if (!nombre || !email || !rol) return res.status(400).json({ error: 'Nombre, email y rol son obligatorios.' });

    const chars = 'ABCDEFGHJKMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz23456789!@#$';
    const password = Array.from({ length: 12 }, () => chars[Math.floor(Math.random() * chars.length)]).join('');

    const { data: authData, error: authError } = await supabaseAdmin.auth.admin.createUser({
        email: email.toLowerCase().trim(), password, email_confirm: true, user_metadata: { nombre, rol }
    });

    if (authError) {
        if (authError.message.includes('already registered')) return res.status(409).json({ error: 'Ya existe un usuario con ese email.' });
        return res.status(500).json({ error: authError.message });
    }

    await supabaseAdmin.from('profiles').upsert({ id: authData.user.id, nombre, rol }).eq('id', authData.user.id);
    res.status(201).json({ id: authData.user.id, email: authData.user.email, nombre, rol, activo: true, _tempPassword: password });
});
...
app.get('/api/v2/operarios', authGuard, async (req, res) => {
    const { data, error } = await supabaseAdmin
        .from('profiles').select('id, nombre, rol, activo')
        .in('rol', ['admin', 'trabajador']).eq('activo', true).order('nombre');
    if (error) return res.status(500).json({ error: error.message });
    res.json(data);
});
...
app.get('/api/v2/tickets', authGuard, async (req, res) => {
    ...
    let query = supabaseAdmin
        .from('tickets_v2')
        .select(`*, empresas(id, nombre), dispositivos(id, nombre, tipo), ticket_asignaciones(user_id, asignado_at), ticket_horas(horas)`)
    ...
    let profileMap = {};
    if (userIds.size > 0) {
        const { data: perfiles } = await supabaseAdmin.from('profiles').select('id, nombre').in('id', [...userIds]);
    ...
    let result = data.map(t => ({
        ...
        horas_totales: (t.ticket_horas || []).reduce((s, h) => s + Number(h.horas), 0),
        horas_transcurridas: t.created_at ? Math.round((Date.now() - new Date(t.created_at).getTime()) / 360000) / 10 : 0
    }));

    if (operario_id) result = result.filter(t => t.ticket_asignaciones?.some(a => a.user_id === operario_id));
...
app.get('/api/v2/tickets/:id', authGuard, async (req, res) => {
    ...
    data.horas_transcurridas = data.created_at
        ? Math.round((Date.now() - new Date(data.created_at).getTime()) / 360000) / 10
        : 0;
...
app.post('/api/v2/tickets', authGuard, async (req, res) => {
    const { empresa_id, dispositivo_id, asunto, descripcion, prioridad, operarios, notas } = req.body;
    if (!empresa_id || !asunto) return res.status(400).json({ error: 'empresa_id y asunto son obligatorios.' });

    const { data: ticket, error } = await supabaseAdmin
        .from('tickets_v2')
        .insert({ empresa_id, dispositivo_id: dispositivo_id || null, asunto, descripcion, notas: notas || null, prioridad: prioridad || 'Media', estado: 'Pendiente', created_by: req.user.id })
        .select().single();

    if (error) return res.status(500).json({ error: error.message });

    await registrarHistorial(ticket.id, req.user.id, 'creacion', `Ticket creado por ${req.user.nombre || req.user.email}`);
...
app.put('/api/v2/tickets/:id', authGuard, async (req, res) => {
    ...
    const { data: old } = await supabaseAdmin.from('tickets_v2').select('*').eq('id', req.params.id).single();
    ...
    if (estado && estado !== old.estado) {
        ...
        await registrarHistorial(req.params.id, req.user.id, 'estado', `Estado: "${old.estado}" → "${estado}"`, { de: old.estado, a: estado });
    }

    if (prioridad && prioridad !== old.prioridad) {
        await registrarHistorial(req.params.id, req.user.id, 'prioridad', `Prioridad: "${old.prioridad}" → "${prioridad}"`, { de: old.prioridad, a: prioridad });
    }

    const { data, error } = await supabaseAdmin.from('tickets_v2').update(updates).eq('id', req.params.id).select().single();
    if (error) return res.status(500).json({ error: error.message });
    res.json(data);
});
...
// FIX: No usar join con profiles directamente (relación ambigua).
// Resolver nombres en una query separada.
// ============================================
app.post('/api/v2/tickets/:id/asignaciones', authGuard, async (req, res) => {
    const { operarios } = req.body;
    if (!operarios?.length) return res.status(400).json({ error: 'Debes proporcionar al menos un operario.' });

    const ticketId = req.params.id;

    // 1) Upsert sin SELECT join para evitar "more than one relation"
    ...
    // 2) Leer las asignaciones sin join
    const { data, error } = await supabaseAdmin
        .from('ticket_asignaciones')
    ...
    // 3) Resolver nombres por separado
    const { data: perfiles } = await supabaseAdmin
        .from('profiles').select('id, nombre').in('id', operarios);
    const profileMap = {};
    perfiles?.forEach(p => { profileMap[p.id] = p.nombre; });

    res.json(data.map(d => ({ ...d, profiles: { id: d.user_id, nombre: profileMap[d.user_id] || '?' } })));
});
...
app.post('/api/v2/tickets/:id/archivos', authGuard, upload.array('files', 10), async (req, res) => {
    ...
    for (const file of files) {
        ...
        const { data } = await supabaseAdmin
            ...
            .insert({ ticket_id: req.params.id, nombre_original: file.originalname, storage_path: storagePath, mime_type: file.mimetype, tamanio: file.size, subido_by: req.user.id })
            .select().single();
...
app.get('/api/v2/archivos/:archivoId/url', authGuard, async (req, res) => {
    const { data: archivo, error } = await supabaseAdmin
        .from('ticket_archivos').select('storage_path, nombre_original, mime_type').eq('id', req.params.archivoId).single();
    if (error || !archivo) return res.status(404).json({ error: 'Archivo no encontrado' });

    const { data: urlData, error: urlError } = await supabaseAdmin.storage
        .from('ticket-archivos').createSignedUrl(archivo.storage_path, 3600);
...
// ============================================
// TICKETS V2 — HORAS (registros manuales)
// ============================================
app.post('/api/v2/tickets/:id/horas', authGuard, async (req, res) => {
    const { horas, descripcion, fecha } = req.body;
    if (!horas || isNaN(horas) || Number(horas) <= 0) return res.status(400).json({ error: 'Horas inválidas.' });

    const { data, error } = await supabaseAdmin
        .from('ticket_horas')
        .insert({ ticket_id: req.params.id, user_id: req.user.id, horas: Number(horas), descripcion, fecha: fecha || new Date().toISOString().split('T')[0] })
        .select().single();

    if (error) return res.status(500).json({ error: error.message });

    await registrarHistorial(req.params.id, req.user.id, 'horas',
        `${req.user.nombre || req.user.email} registró ${horas}h${descripcion ? ': ' + descripcion : ''}`, { horas, descripcion });

    res.status(201).json({ ...data, profiles: { id: req.user.id, nombre: req.user.nombre || req.user.email } });
});

app.delete('/api/v2/horas/:horaId', authGuard, async (req, res) => {
    const { data: hora } = await supabaseAdmin.from('ticket_horas').select('user_id').eq('id', req.params.horaId).single();
    if (!hora) return res.status(404).json({ error: 'No encontrado' });
    if (hora.user_id !== req.user.id && req.user.rol !== 'admin') return res.status(403).json({ error: 'Sin permisos.' });
    await supabaseAdmin.from('ticket_horas').delete().eq('id', req.params.horaId);
    res.json({ ok: true });
});

// ============================================
// TICKETS V2 — ESTADÍSTICAS (solo admin)
// ============================================
app.get('/api/v2/estadisticas/resumen', authGuard, adminGuard, async (req, res) => {
    const { data, error } = await supabaseAdmin.from('tickets_v2').select('estado, prioridad, created_at');
    if (error) return res.status(500).json({ error: error.message });
...
app.get('/api/v2/estadisticas/operarios', authGuard, adminGuard, async (req, res) => {
    ...
    let query = supabaseAdmin.from('tickets_v2')
        .select(`id, estado, prioridad, created_at, completed_at, ticket_asignaciones(user_id), ticket_horas(user_id, horas)`)
    ...
    const { data: todos } = await supabaseAdmin.from('tickets_v2').select('id, estado, ticket_asignaciones(user_id)');
    ...
    tickets?.forEach(ticket => {
        ticket.ticket_asignaciones?.forEach(a => {
            if (!map[a.user_id]) map[a.user_id] = { id: a.user_id, nombre: profileMap[a.user_id] || '?', tickets_totales: 0, tickets_completados: 0, tickets_pendientes: 0, horas_totales: 0, tiempos_resolucion: [] };
            map[a.user_id].tickets_totales++;
            map[a.user_id].tickets_completados++;
            if (ticket.created_at && ticket.completed_at) {
                map[a.user_id].tiempos_resolucion.push((new Date(ticket.completed_at) - new Date(ticket.created_at)) / 3600000);
            }
        });
        ticket.ticket_horas?.forEach(h => { if (map[h.user_id]) map[h.user_id].horas_totales += Number(h.horas); });
    });
    ...
    res.json(Object.values(map).map(op => ({
        ...op,
        tiempo_promedio_horas: op.tiempos_resolucion.length ? op.tiempos_resolucion.reduce((a, b) => a + b, 0) / op.tiempos_resolucion.length : null
    })));
});

app.get('/api/v2/estadisticas/empresas', authGuard, adminGuard, async (req, res) => {
    const { desde, hasta } = req.query;
    let query = supabaseAdmin.from('tickets_v2').select('empresa_id, estado, prioridad, created_at, empresas(nombre)');
    if (desde) query = query.gte('created_at', desde);
    if (hasta) query = query.lte('created_at', hasta + 'T23:59:59');
    ...
    const map = {};
    data?.forEach(t => {
        if (!map[t.empresa_id]) map[t.empresa_id] = { id: t.empresa_id, nombre: t.empresas?.nombre, total: 0, pendientes: 0, en_curso: 0, completados: 0, facturados: 0, urgentes: 0 };
